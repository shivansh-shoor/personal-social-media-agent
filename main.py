...
app = FastAPI(title="Personal Social Memory Agent", version="1.0.0")
...
TASK_BANK = {
    "fact_extraction": [
        ...
        {"q": "What is Shivansh's internship company?", "key_facts": ["ecofuelz"]}
    ],
    "thread_summary": [
        {"q": "Summarize the Goa trip planning", "gold_keywords": ["goa", "priya", "rahul", "beach", "hotel"]}
    ],
    "multi_hop_qa": [
        {"q": "What gift is planned for Priya's birthday and who contributes how much?", "key_facts": ["necklace", "tanishq", "2000"]}
    ]
}

def _build_obs(tt: str, q: dict) -> str:
    return f"TASK: {tt.upper()}\nQUESTION: {q['q']}\n\nAnswer based on the social memory data provided."
...
@app.get("/health")
async def health():
    return {"status": "ok", "env": "personal-social-memory-agent"}
